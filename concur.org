...
*** title: when handling a failure, don't just retry
    ...
    - restarting the problematic component
    ...
    (but also I want to say that busy loops are bad... hmm...)

    (but I guess event loops are loops anyway... they're just not busy...)
    (ok it's that you're waiting and retrying for the program to exit, or something.
    and each time you step it forward, whatever)
* another note
  not waking up pointlessly in busy loops allows your process to be paged out,
  and also avoids wasting energy.

  both are good.
  the former is great if you have lots of different processes which aren't actively running (like tabs in a browser)
