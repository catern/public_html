...
Heartbeat-timeout-based failure detection is also very slow, because you have to wait for the timeout.
This will bog down your system and cause severe issues when you actually hit failures.
(can't find stuff about this. it's kind of like bufferbloat? no trio stuff either)
** don't add timeouts
An arbitrary timeout on an operation is usually wrong;
you'll spontaneously fail in a different network or load environment,
...
Detect failures by using communication mechanisms with built-in failure detection,
not by adding timeouts.
...
Prefer APis where you send a request and don't get a response until the operation is complete.
That's easier to use and easier to get right.

Maybe there's no data availalbe when you send the request,
or there's some other transient issue.
Still, just send the request, and wait however long (minutes, hours) that it might take for the response to come back.

The alternative is a readiness-based API;
where you wait for the right state for the operation,
and only then do it.

A readiness-based API can be implemented more cheaply,
but it's harder to use correctly.
In particular, in a completion API,
...
You might get lucky, maybe even 99.99% of the time,
but under load or with bad scheduling,
you'll eventually get unlucky and cause breakages.
...
Achieving good support for such waiting will often require a high-quality service implementation
which exposes some kind of notifications about when it changes state.
If you're writing a client,
don't be afraid of adding support for these kinds of status updates to the server.

Some readiness-based APIs:
- starting up servers and waiting for them to be ready
- waiting for O_NONBLOCK fds to be readable
- state changes in processes
  (what's a good normal-person example? I guess I can just say "a certain stock being no longer available for trading")
...
And remember that some services forward operations to other services;
they may be incorrectly coded.
*** don't send ready-notifications too soon
Make sure that when they're saying "X is ready",
that means any requests which come *instantaneously* at that point will succeed.
X shouldn't say "X is ready" until that's true.
...
if they send operations fast enough.
** don't use retries
You should arrange things so that you don't need to add retries.
...
For example, you can just let the exception propagate,
or just crash your process and let someone restart it.

But sometimes, retrying is necessary for correctness.
For example,
TOCTOU issues can require retrying when using a readiness interface,
...
- the server said it was ready, but then it failed again before executing the client's operation,
