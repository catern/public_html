#+title: No more DSLs: Using typed code to deploy distributed systems directly
#+HTML_HEAD: <style type="text/css">pre.src {background-color: #303030; color: #ffffff;} pre.src-python:before { color: #000000; }</style>
* Introduction
At $DAYJOB we have
a sophisticated collection of libraries for [[http://catern.com/run.html][running our distributed system]], called "integration libraries".
