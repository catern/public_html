...
You can write a distributed system as a single program in a general-purpose language.
Then you can run that program to [[http://catern.com/run.html][run the full system]],
for production or for testing the system [[http://catern.com/usertests.html][against its users]].
I call this a "single-program system".
In this article, I describe the approach used to do this at $DAYJOB,
and show some working example programs.
Although $DAYJOB's single-program systems are proprietary,
...
(though doing anything actually useful with the system might be [[https://www.lesswrong.com/posts/Wa2hASzbxyvutHJff/total-horse-takeover][hard]]).
