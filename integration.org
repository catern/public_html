...
and returns a value that can be passed as an argument (a dependency) for other services.
...
class Exampled:
    def __init__(self, path: rsyscall.Path) -> None:
        self.path = path
...
async def start_exampled(
    ...
    await child_thread.mkdir(path)
    await child_thread.chdir(path)
    ...
    # it communicates with the world only by creating files under `path'
    return Exampled(path)
#+end_src
...
I just need to know the earliest valid byte...
** tm007
*** TODO my reaction
